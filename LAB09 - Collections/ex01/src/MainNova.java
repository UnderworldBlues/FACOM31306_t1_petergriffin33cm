...
public class MainNova {
    ...
    List<Estudante> estudanteArrayList = new ArrayList<>();
        estudanteArrayList.add(est1);
        estudanteArrayList.add(est2);
        estudanteArrayList.add(epm);

        System.out.println("\n===== Estudantes (ArrayList) =====");

    mostrarEstudantes(estudanteArrayList);

    // Fila de estudantes
    Queue<Estudante> estudanteQueue = new LinkedList<>();
        estudanteQueue.add(est1);
        estudanteQueue.add(est2);
        estudanteQueue.add(epm);

        System.out.println("\n===== Estudantes (LinkedList) =====");

    mostrarEstudantes(estudanteQueue);

    // Ordenando a lista de estudantes por nome
        System.out.println("\n===== Estudantes ordenados por nome =====");
        Collections.sort(estudanteArrayList,Comparator.comparing(Estudante::getNome));

    mostrarEstudantes(estudanteArrayList);

}

    // Métodos existentes em seu código
    public static void mostrarEstudantes(List<Estudante> lista) {
        for (Estudante e : lista) {
            e.print();
        }
    }

    public static void mostrarEstudantesPesquisadores(ArrayList<? extends EstudantePosGrad> lista) {
        for (EstudantePosGrad e : lista) {
            e.print();
        }
    }
